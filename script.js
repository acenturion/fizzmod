...
function mapCustomizado(container, operation) {
  return container.map(operation);
}
/**
...
Array.prototype.mapCustomizado = function (operation) {
...
/**
 * 5) Los miembros de trabajo especificados en el siguiente objeto usan su nombre como indice y 
 * su edad como valor. Separa los miembros mayores de 40 años y menores de 25 años en un array y 
 * todo el resto en un segundo array. 
...
let miembrosMayoresCuarenta = getMayoresDe(40);
let miembrosMenoresVeinticinco = getMenoresDe(25);

function getMayoresDe(edad) {
  let result = [];
  Object.entries(miembros).sort().forEach(e => {
    if (e[1] > edad) {
      result.push(e[0]);
    }
  });
